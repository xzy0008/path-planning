...
%coordinate offset: solar house 
east_offset=641747.74847656;
north_offset=3606773.38784639;

n=10;m=8;
xmin=-15; xmax=5; ymin=20; ymax=55;
dir='C:\\Users\\zhongyuan\\Desktop\\experiment\\';

if 0
%%write tmp
filename=sprintf('%stsp%d_%d.tmp',dir,n,m); 
...
a =[[1:n]' (xmax-xmin)*rand(n,1)+xmin (ymax-ymin)*rand(n,1)+ymin];
dlmwrite(filename, a,'-append','delimiter', ' ', 'precision', '%.2f'); 
fclose(fid);

%%write tsp
filename=sprintf('%stsp%d_%d.tsp',dir,n,m);
fid = fopen(filename,'w');
dlmwrite(filename, []);
fprintf(fid,'NAME: tsp%d_%d\n', n,m);
fprintf(fid,'COMMENT: %d-tsp problem (Padberg/Rinaldi)\n',n);
...
%%write par_tsp
filename=sprintf('%stsp%d_%d.par',dir,n,m);
fid = fopen(filename,'w');
fprintf(fid,'PROBLEM_FILE= %stsp%d_%d.tsp\n',dir,n,m);
fprintf(fid,'OPTIMUM 378032\n');
fprintf(fid,'MOVE_TYPE = 5\n');
fprintf(fid,'PATCHING_C = 3\n');
fprintf(fid,'PATCHING_A = 2\n');
fprintf(fid,'RUNS = 10\n');
fprintf(fid,'OUTPUT_TOUR_FILE = %soutput_tsp%d_%d.txt\n',dir,n,m);
fclose(fid);
end 

%process LKH to compute
filename=sprintf('lkh %stsp%d_%d.par',dir,n,m);
system(filename);
...
filename=sprintf('%soutput_tsp%d_%d.txt',dir, n, m);
fid = fopen(filename, 'r');
for i = 1:6
    tline = fgetl(fid);
end
seq = fscanf(fid, '%g', [1,n]);
seq = seq';
fclose(fid);
...
%     h=plotppCircle(initial_nodes(i).circle,'b');
    h=plotppCircle(ppCircle(initial_nodes(i).circle.x,initial_nodes(i).circle.y,1.0),'b');
end
...
filename=sprintf('%stestdata_%dm_%s.txt',dir,turn_radius,date);
fid = fopen(filename,'w');
fprintf(fid,'%.8f\n', turn_radius);

for i=1:length(nodes)
    fprintf(fid,'%.8f %.8f %.8f %.8f %.8f\n',nodes(i).dockConfiguration(1)+east_offset,...
        nodes(i).dockConfiguration(2)+north_offset,...
