function out = ppSurveyTspOptimal(startPoints, endPoints, startDirection, minRadius, pathSpeed, eastRef, northRef)
...
for i=1:numPasses 
    ...
    passes(2*i-1).startPoint.heading=startDirection;
    passes(2*i-1).endPoint.x=endPoints.x(i);
    passes(2*i-1).endPoint.y=endPoints.y(i);
    passes(2*i-1).endPoint.heading=startDirection;
    ...
    passes(2*i).startPoint.heading=startDirection+pi;
    passes(2*i).endPoint.x=startPoints.x(i);
    passes(2*i).endPoint.y=startPoints.y(i);
    passes(2*i).endPoint.heading=startDirection+pi;
...
if isfixed
    ...
    cost(:,numVirPasses)=2147;
    cost(numVirPasses,1)=-1000;
    cost(numVirPasses-1,numVirPasses)=-1000;
else
    cost(numVirPasses,1:end-1)=-1000;
    cost(1:end-1,numVirPasses)=-1000;
end
